...
function Sidebar({ 
  ...
  const sidebarItems = [
    { icon: Home, label: "Markets", active: true },
    { icon: PieChart, label: "Portfolio" },
    { icon: Plus, label: "Create Market" },
      ...
      `}>
        ...
        <button
          ...
          className={`
            ...
            ${isDarkMode ? 'bg-gray-800 border-gray-700 hover:bg-gray-700' : 'bg-gray-100 border-gray-300 hover:bg-gray-200'}
            border
          `}
        >
              ...
              {isConnected ? (
                <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className={chainId === pepuTestnet.id ? 'text-emerald-500' : 'text-yellow-500'}>
                      {chainId === pepuTestnet.id ? 'PEPU Testnet' : 'Other'}
                    </span>
                  </div>
                  {chainId === pepuTestnet.id && balance && (
                    <div className="flex items-center justify-between text-xs mt-1">
                      <span className={isDarkMode ? 'text-gray-400' : 'text-gray-500'}>Balance:</span>
                      <span className="font-medium">{parseFloat(balance.formatted).toFixed(2)} PEPU</span>
                    </div>
                  )}
                </div>
              ) : (
